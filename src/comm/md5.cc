...
string Md5::Md5HashString(const char* inBuffer, int length) {
    static char md5buf[16];
    static char md5strbuf[64]; //32+1 bytes is ok. but we give it more.

    Md5HashBuffer(md5buf, inBuffer, length);
    for (int i = 0; i < 16; ++i) {
        sprintf(md5strbuf+i*2, "%02x", (unsigned char)md5buf[i]);
    }
    return md5strbuf;  
}

void Md5::Md5HashBuffer( BYTE *outBuffer, const BYTE *inBuffer, int length)
{
